<div class="container-fluid">
    <br>
<div class="row" id="makeup_div">Basic API request with CURL using https://makeup-api.herokuapp.com/
    <br>
    <strong>Results: <?php echo isset($data) ? count($data) : ''; ?></strong>
</div>
<br>
<form action="/makeup" method="GET" name="makeup_form" class="makeup_form">
    <!-- <div class="" id="makeup_div">
            <input class="form-check-input" type="checkbox" name="product_type" id="makeup_div" onclick="return submitForm()" <?php // echo ((isset($_GET['product_type'])) ? 'checked' : ''); 
                                                                                                                                ?>>
            <label class="form-check-label" for="product_type">Type</label>
            <input class="form-check-input brand" type="checkbox" name="brand" id="makeup_div" onclick="return submitForm()" <?php // echo ((isset($_GET['brand'])) ? 'checked' : ''); 
                                                                                                                                ?>>
            <label class="form-check-label" for="brand">Brand</label>
        </div> -->
    <div class="inline-block" id="makeup_div">


        <div class="containerinline-block">
            <div class="d-flex justify-content-start inline-block">
                <label for="type">Type:&nbsp;</label>

                <select class="form-group dropdown form-control w-50 inline-block" name="product_type" id="makeup_type">
                    <?php echo isset($_GET['product_type']) ? '<option value="'.$_GET['product_type'].'">'.urldecode($_GET['product_type']).'</option>' : NULL?>
                ...
                <div class="container inline-block">
                    <div class="d-flex justify-content-start inline-block">
                        ...
                        <select class="form-group dropdown form-control w-60 inline-block " name="brand" id="makeup_type">
                            <?php echo isset($_GET['brand']) ? '<option value="'.$_GET['brand'].'">'.ucwords(urldecode($_GET['brand'])).'</option>' : NULL?>
                            <option value="All">All</option>
